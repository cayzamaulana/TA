...
namespace OneClickLocalization.Editor
{
    ...
    public class OCLSetupWindow : EditorWindow
    {
        ...
        private void CreateParameters()
        {
            ...
            DataManager.editorSetup.active = EditorGUILayout.Toggle(new GUIContent("Activate Localization", "If unchecked, One Click Localization will have no effect on the project"), DataManager.editorSetup.active);

            EditorGUILayout.Separator();

            DataManager.editorSetup.forceLanguage = !(EditorGUILayout.Toggle("Use system's language", !DataManager.editorSetup.forceLanguage));

            GUI.enabled = DataManager.editorSetup.forceLanguage;
            DataManager.editorSetup.forcedLanguage = (SystemLanguage)EditorGUILayout.EnumPopup("Force language", DataManager.editorSetup.forcedLanguage);
            GUI.enabled = true;

            DataManager.editorSetup.useDefaultLanguageForNullValues = EditorGUILayout.Toggle(new GUIContent("Default language for nulls", "If a translation has a null value, default language value will be returned by API"), DataManager.editorSetup.useDefaultLanguageForNullValues);
        ...
        private void CreateSetupSettings()
        {
            ...
            DataManager.addAdapters= EditorGUILayout.Toggle(new GUIContent("Add OCL component", "If checked, OCLComponentAdapter component will be added to objects with supported comps, making the object localization ready."), DataManager.addAdapters);

            // Extract comp data
            DataManager.extractLocalizationData = EditorGUILayout.Toggle(new GUIContent("Extract data", "If checked, supported types are extracted from object components to the localization list."), DataManager.extractLocalizationData);
            ...
            DataManager.selectedSupportedTypes = EditorGUILayout.MaskField(new GUIContent("Supported types", "Select types to be handled by the setup process."), DataManager.selectedSupportedTypes, supportedTypesNames);
            ...
            // Scenes 
            DataManager.parseScenes = EditorGUILayout.BeginToggleGroup(new GUIContent("Parse Scene objects", "If checked, selected scenes will be parsed during setup process."), DataManager.parseScenes);
            EditorGUI.indentLevel++;

            EditorGUILayout.LabelField("Only scenes from the build settings 'Scenes in build' list are available.", EditorStyles.helpBox);
            ...
            DataManager.selectedScenes = EditorGUILayout.MaskField("Selected scenes", DataManager.selectedScenes, scenesNames);

            GUIContent inactiveGUIContent = new GUIContent("Parse inactives", "If checked, inactive objects in scenes will be used for the setup process");
            DataManager.parseInactivesInScenes = EditorGUILayout.Toggle(inactiveGUIContent, DataManager.parseInactivesInScenes);
            ...
            // Prefabs 
            DataManager.parsePrefabs = EditorGUILayout.BeginToggleGroup(new GUIContent("Parse Prefab assets", "If checked, prefab assets will be parsed during setup process."), DataManager.parsePrefabs);
            EditorGUI.indentLevel++;
            ...
            inactiveGUIContent = new GUIContent("Parse inactives", "If checked, inactive objects in prefabs will be used for the setup process");
            DataManager.parseInactivesInPrefabs = EditorGUILayout.Toggle(inactiveGUIContent, DataManager.parseInactivesInPrefabs);
        ...
        private void CreateLanguages()
        {
            ...
            SystemLanguage modifiedDefaultLanguage = (SystemLanguage)EditorGUILayout.EnumPopup(new GUIContent("Default", "The language used before applying any localization"), DataManager.editorSetup.defaultLanguage);
            ...
            if(modifiedDefaultLanguage != DataManager.editorSetup.defaultLanguage)
            {
                ...
                {
                    EditorUtility.DisplayDialog("Change default language", "You can't specify a default language that is used.\n\nRemove " + modifiedDefaultLanguage + " to use it as default language.", "OK");
                }
                else
                {
                    // Valid modification
            ...
            else
            {
                ...
                for(int i = DataManager.editorSetup.languages.Count - 1; i >= 0; i--)
                {
                    ...
                    GUILayout.Label(idsTranslatedForLang + "/" + idsCount + " translated", GUILayout.MinWidth(120), GUILayout.Height(25));
                    if (GUILayout.Button(DataManager.closeIconTex, GUILayout.Width(25), GUILayout.Height(25)))
                    {
                        if (EditorUtility.DisplayDialog("Remove language", "Are you sure you want remove " + lang.ToString() + " language?\nYou'll lose all localizations for this language!", "Do it! YES!", "Wow NO! Stop!"))
                        {
            ...
            EditorGUILayout.BeginHorizontal();
            addLanguageValue = (SystemLanguage)EditorGUILayout.EnumPopup(addLanguageValue, GUILayout.ExpandWidth(true));
            ...
            if (GUILayout.Button(new GUIContent( " Edit Localizations", DataManager.editLocalizationsIconTex), mainButtonStyle, GUILayout.Height(32), GUILayout.Width(300))){
        ...
        private void CreateAutomaticSetup()
        {
            ...
            GUI.enabled = (DataManager.addAdapters || DataManager.extractLocalizationData) && (DataManager.parsePrefabs || DataManager.parseScenes);
            ...
            if (GUILayout.Button(setupGUIContent, mainButtonStyle, GUILayout.Height(50), GUILayout.Width(300), GUILayout.MinWidth(100)))
            {
                ParseResult res = Parse(DataManager.parseScenes, DataManager.parsePrefabs, DataManager.extractLocalizationData, DataManager.addAdapters, false);
                if (res != null)
                {
                    string comps = (res.componentsAdded + res.componentsAlreadyExist) + " supported components found.";
                    string extractedStr = res.stringsExtracted + " strings extracted (" + res.stringsDuplicates + " duplicates skipped)";
                    string extractedSprites = res.spritesExtracted + " Images(Sprite) extracted (" + res.spritesDuplicates + " duplicates skipped)";
                    string extractedTextures = res.texturesExtracted + " Images(Texture) extracted (" + res.texturesDuplicates + " duplicates skipped)";
                    string extractedAudioClips = res.audioClipExtracted + " Audio(AudioClip) extracted (" + res.audioClipDuplicates + " duplicates skipped)";
                    string setupComps = res.componentsAdded + " OCLComponentAdapter added (" + res.componentsAlreadyExist + " already present)";
                    EditorUtility.DisplayDialog("Setup Project Result", res.scenesParsed + " scenes parsed\n" + res.prefabsParsed + " prefabs parsed" + "\n" + "\n" + comps + "\n\n" + extractedStr + "\n" + extractedSprites + "\n" + extractedTextures + "\n" + extractedAudioClips + "\n\n" + setupComps, "OK");
                }
                else
                {
        ...
        private void CreateTranslatorConfig()
        {
            ...
            EditorGUILayout.SelectableLabel("Documentation : https://msdn.microsoft.com/en-us/library/mt146806.aspx");
            
            string newClientId = EditorGUILayout.TextField("Client Id", DataManager.translatorClientId);
            string newClientSecret = EditorGUILayout.TextField("Client Secret", DataManager.translatorClientSecret);

            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(150);
            if (GUILayout.Button(new GUIContent("Set default credentials", "Default OCL account for Microsoft Translator is public, so you may encounter \"Quota reached error\", use your own."), GUILayout.ExpandWidth(false)))
            {
                GUI.FocusControl(null);
                newClientId = DataManager.defaultTranslatorClientId;
                newClientSecret = DataManager.defaultTranslatorClientSecret;
            }
            EditorGUILayout.EndHorizontal();

            DataManager.translatorClientId = newClientId;
            DataManager.translatorClientSecret = newClientSecret;

            EditorGUILayout.EndVertical();
             
            if(DataManager.translatorClientId != newClientId || DataManager.translatorClientSecret != newClientSecret)
            {
                DataManager.translator.SetCredentials(newClientId, newClientSecret);
            }
        ...
        private void CreateReset()
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.inspectorDefaultMargins);
            if (GUILayout.Button(new GUIContent("Reset OCL", "Removes all OCLComponentAdapters from scenes and prefabs, reset localizations and languages"), GUILayout.ExpandWidth(false)))
            {
                if (EditorUtility.DisplayDialog("Reset OCL", "OneClickLocalization will reset to its default configuration.\n   - All OCLComponentAdapter will be searched and removed from all scenes and prefabs objects.\n   - All localizations will be deleted\n\n Are you sure?", "Yeah !", "Are you crazy?! Stop, you fool!"))
                {
                    if (EditorUtility.DisplayDialog("Reset OCL - Miss click security", "100% sure? No regrets?", "Yes, reset all", "No, don't do anything"))
                    {
                        GUI.FocusControl(null);
                        ParseResult result = Parse(true, true, false, false, true);
                        if(result != null)
                        {
                            ...
                            EditorUtility.DisplayDialog("Reset Result", result.componentsRemoved + " OCLComponentAdapter removed\n\nSetup reseted to default.", "OK");
                        }
                        else
                        {
        ...
        private ParseResult Parse(bool parseScenes, bool parsePrefabs, bool extractData, bool addAdapters, bool reset)
        {
            ...
            try
            {
                ...
                if (parsePrefabs)
                {
                    ...
                    {
                        EditorUtility.DisplayProgressBar("Setup progress", "Parse prefab : " + prefab.name, progressStatus + progressStep);
                        res.prefabsParsed++;
                        ...
                        foreach (Component comp in components) { 
                            ...
                            if (compGameObject == null || compGameObject.hideFlags == HideFlags.NotEditable || compGameObject.hideFlags == HideFlags.HideAndDontSave)
                                continue;
                ...
                if (parseScenes)
                {
                    ...
                    {
                        EditorUtility.DisplayProgressBar("Setup progress", "Parse scene : " + scene.path, progressStatus + progressStep);
        ...
        private bool IsCompSupported(Component comp, List<Type> selectedSupportedTypes)
        {
            ...
            string[] includeTypes = DataManager.parseIncludeComps.Split(DataManager.includeExcludeCompsSeparator.ToCharArray());
            string[] excludeTypes = DataManager.parseExcludeComps.Split(DataManager.includeExcludeCompsSeparator.ToCharArray());
        ...
        private void ExtractCompData(Component comp, ParseResult parseResult)
        {
            ...
            if (newId != null && !(newId.Equals("")))
            {
                if (!DataManager.editorSetup.HasId(newId))
                {
                    ...
                    else if (newId is Sprite)
                        parseResult.spritesExtracted++;
                    else if (newId is Texture)
                        parseResult.texturesExtracted++;
                    else if (newId is AudioClip)
